...
def query_to_cur(dbh, query, args):
   ...
   if args.debug : print >> sys.stderr, datetime.datetime.strftime(datetime.datetime.now(),"%D %H:%m:%S"), query
   t0=time.time()
   cur = dbh.cursor()
   cur.execute(query)
...
def printPrettyFromCursor (cur, args):
       ...
       if args.header: rows.append([item[0] for item in cur.description])
...
def printCSVFromCursor(cur, args) :
   ...
   if not cur.description: return #nothing to print.
...
def main():
	...
	parser.add_argument('query',help='query to execute (or -, read from standard in)', default=None)
        ...
        if not args.log :  args.log = os.getenv("DESPYDB_QUERY_LOG") # Undefined -> returns None
        query(args)
